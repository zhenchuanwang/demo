...
export default {
    ...
    methods: {
        ...
        getListData(queryData) {
            return this.$api.Sp.ShopLevel.data(queryData).then(json => {
                const res = json.data;
        ...
        onTableEdit(row) {
            const { formModel } = this.addDialog;
            this.$api.Sp.ShopLevel.getDetail({ id: row.id }).then(json => {
        ...
        onTableDelete(ids) {
            this.$confirm('是否确定要删除！', '温馨提示', {
                ...
                this.$api.Sp.ShopLevel.delete({ ids }).then(json => {
                    const res = json.data;
                    this.$message({
                        message: res.msg,
                        type: 'success',
